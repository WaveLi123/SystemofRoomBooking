package entities;
import database.Management;

import java.sql.*;
import java.util.ArrayList;
import java.util.List;


public class Route 
{
//	create table Route(
//			编号 varchar(23),
//			始发站 varchar(23),
//			终点站 varchar(23),
//			经站数 varchar(23),
//			所需时长 varchar(23)	
//	)
	private static final long serialVersionUID = 20101025L;
	//data
	public String 编号;
	public String 始发站; 
	public String 终点站; 
	public String 经站数; 
	public String 所需时长; 
	
	//带参数的构造函数
	public Route(String str0,String str1,String str2,String str3,String str4)
	{
		this.编号 = str0;
		this.始发站 = str1;
		this.终点站 = str2;
		this.经站数 = str3;
		this.所需时长 = str4;
	}
	//不带参数的构造函数
	public Route(){}
	
	//获取相关信息
	public void get_info(String str0,String str1,String str2,String str3,String str4){
		this.编号 = str0;
		this.始发站 = str1;
		this.终点站 = str2;
		this.经站数 = str3;
		this.所需时长 = str4;
	}
	
	//将信息添加到数据库中
	public void storeToDB() throws SQLException, ClassNotFoundException
	{
		String sqlS="insert into Route(编号,始发站,终点站,经站数,所需时长)"
					+"values("
					+"'"+this.编号+"'"+","
					+"'"+this.始发站+"'"+","
					+"'"+this.终点站+"'"+","
					+"'"+this.经站数+"'"+","
					+"'"+this.所需时长+"'"
					+")";				
		Management operate=new Management();
		operate.stmt.executeUpdate(sqlS);
	}
	//更新数据库中的信息
	public void updateDB() throws SQLException, ClassNotFoundException
	{
		String sqlS="update Route "
					+"set "
					+"编号 = "+"'"+this.编号+"'"+","
					+"始发站 = "+"'"+this.始发站+"'"+","
					+"终点站 = "+"'"+this.终点站+"'"+","
					+"经站数 = "+"'"+this.经站数+"'"+","
					+"所需时长 = "+"'"+this.所需时长+"'"
					+" where 编号  = "+"'"+this.编号+"'";					
		Management operate=new Management();
		operate.stmt.executeUpdate(sqlS);
	}
	//删除数据库中的信息
	public void deleteDB(String num) throws SQLException, ClassNotFoundException
	{
		String sqlS="delete "
					+"from Route "
					+" where 编号 = "+"'"+num+"'";					
		Management operate=new Management();
		operate.stmt.executeUpdate(sqlS);
	}
	
	//获取信息
	public String[] getinfo(String num) throws SQLException, ClassNotFoundException{
		String sqlS="select * "
				+"from Route "
				+" where 编号 = "+"'"+num+"'";				
		Management operate = new Management();
		operate.rs = operate.stmt.executeQuery(sqlS);		
		ResultSetMetaData rsmd=operate.rs.getMetaData();	//创建结果集对象
		
		int colCount = rsmd.getColumnCount();		//得到列数
		String[] name = new String[colCount];
		operate.rs.beforeFirst();
		
		String[] value = new String[colCount];		

		ArrayList<String> ar = new ArrayList<String>();  		
		while(operate.rs.next())		//得到各行的属性值
		{			
			for(int i=1;i<=colCount;i++){
				value[i-1]=operate.rs.getString(i);			
			}										
		}
		operate.rs.close();	
		return value;
	}
	
	//获取全部信息
	public List<Route> getallinfo() throws SQLException, ClassNotFoundException{
		String sqlS="select * "
				+"from Route";							
		Management operate = new Management();
		operate.rs = operate.stmt.executeQuery(sqlS);		
		ResultSetMetaData rsmd=operate.rs.getMetaData();	//创建结果集对象
		
		int colCount = rsmd.getColumnCount();		//得到列数
		String[] name = new String[colCount];
		operate.rs.beforeFirst();
		
		String[] value = new String[colCount];		

		List<Route> ar = new ArrayList<Route>();  		
		while(operate.rs.next())		//得到各行的属性值
		{			
			for(int i=1;i<=colCount;i++){
				value[i-1]=operate.rs.getString(i);			
			}
			Route temp = new Route(value[0],value[1],value[2],value[3],value[4]);
			ar.add(temp);
		}
		operate.rs.close();	
		return ar;
	}
	//获取产品信息
	public String[] getkeyinfo() throws SQLException, ClassNotFoundException{
		Management operate = new Management();
		operate.display("Route");
		ResultSetMetaData rsmd=operate.rs.getMetaData();	//创建结果集对象
		int colCount=rsmd.getColumnCount();		//得到列数
		String[] name=new String[colCount];
		operate.rs.beforeFirst();
		
		String[] value=new String[colCount];		

		ArrayList<String> ar = new ArrayList<String>();  		
		while(operate.rs.next())		//得到各行的属性值
		{			
			for(int i=1;i<=colCount;i++){
				value[i-1]=operate.rs.getString(i);			
			}							
			ar.add(value[0]);
		}
		operate.rs.close();
		String [] t_value = new String[ar.size()];
		int j = 0;
		for(int p = 0; p< ar.size(); p ++){
			t_value[p] = ar.get(p);			
		}
		return t_value;
	}
	//added by jkd,4st Jan in 2014
	public Object[] formatCustomerBasicalData(Route temp){    
	  	//"类型","编号","座位数","线路编号"    	
	  	Object [] str =new Object[5];
	  	str[0] = temp.编号;
	  	str[1] = temp.始发站;
	  	str[2] = temp.终点站;
	  	str[3] = temp.经站数;
	  	str[4] = temp.所需时长;
	  	return str;
	}
	public Object[][] getAllData() throws ClassNotFoundException, SQLException {	
		Object[][] temp =new Object[1][5];
		if(getallinfo().size()>0){
			temp = new Object[getallinfo().size()][5];
			Route t = null;
			for(int i=0;i<getallinfo().size();i++){
				t = getallinfo().get(i);
				temp[i] = formatCustomerBasicalData(t);
			}
		}		
		return temp;
	}		 
}